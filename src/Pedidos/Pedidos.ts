...
class Pedidos {
    ...
    async listaDePedidos(): Promise<Pedidotype[]> {
        ...
        const pedidos = await arquivoEmjson.lerAquivos(caminhoPedido) as Pedidotype[]

        const newPedidos = pedidos.filter((pedido, index, self) => {
            if (typeof pedido.número_item !== 'number') {
                throw new Error("O valor do 'número_item' não corresponde ao tipo descrito")
            }
        ...
        const pedidosEmOrdemCrescente = newPedidos.sort( (a, b) => a.número_item - b.número_item)
